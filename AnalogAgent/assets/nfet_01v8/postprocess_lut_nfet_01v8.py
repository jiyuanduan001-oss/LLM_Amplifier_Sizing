...
for corner in corners:
    for temp in temps:
        indir = os.path.join(base_path, corner, temp, "initial")
        outdir = os.path.join(base_path, corner, temp, "processed")
        os.makedirs(outdir, exist_ok=True)

        files = sorted(glob.glob(os.path.join(indir, f"gmid_{device}_L*n.txt")),
            ...
            with open(filename, 'r') as f:
                for line in f:
                    ...
                    try:
                        ...
                        if abs(id_val) < 1e-15 or abs(gm) < 1e-18:
                            continue

                        _gm_id.append(gm / id_val)
                        _gm_gds.append(gm / gds if abs(gds) > 1e-18 else 0)
                        _id_W.append(id_val / W)
                        _ft.append(gm / (2 * np.pi * cgg) if abs(cgg) > 1e-25 else 0)
                        _cgg_W.append(cgg / W)
